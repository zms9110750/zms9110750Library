...
public class WarframeMarketClient([FromKeyedServices(nameof(WarframeMarketClient))] HttpClient http, IFusionCache fusion) : IAsyncDisposable
{
    ...
    public async ValueTask<ItemShort> GetItemByIndexAsync(string index, CancellationToken cancellation = default)
    {
        ...
        return await Fusion.GetOrDefaultAsync<ItemShort>("item:" + slug, token: cancellation) ?? throw new ArgumentException("Item not find");
    }
    ...
    {
        return GetSkippingDistributedCacheAsync<Response<Version>>("https://api.warframe.market/v2/versions", cancellation);
    }
    ...
    {
        var items = await GetSkippingDistributedCacheAsync<Response<ItemShort[]>>("https://api.warframe.market/v2/items", cancellation);
        ConcurrentBag<string> trie = [];
        await Parallel.ForEachAsync(items.Data, cancellation, async (item, can) =>
            {
                await Fusion.SetAsync("item:" + item.Slug, item, tags: [nameof(Item).ToLower()], token: can);
                await Fusion.SetAsync("index:" + item.Slug, item.Slug, tags: [nameof(Item).ToLower(), "index", nameof(item.Slug).ToLower()], token: can);
                await Fusion.SetAsync("index:" + item.Id, item.Slug, tags: [nameof(Item).ToLower(), "index", nameof(item.Id).ToLower()], token: can);

                trie.Add(item.Slug);
                trie.Add(item.Id);
                foreach (var i18n in item.I18n)
                {
                    await Fusion.SetAsync("index:" + i18n.Value.Name, item.Slug, tags: [nameof(Item).ToLower(), "index", nameof(item.I18n).ToLower(), i18n.Key.ToString().ToLower()], token: can);
                    trie.Add(i18n.Value.Name);
                }
            });
        await Fusion.SetAsync(nameof(Trie).ToLower(), trie, tags: [nameof(Item).ToLower(), "trie"], token: cancellation);
        await Fusion.SetAsync(nameof(Version).ToLower(), items.ApiVersion, tags: [nameof(Item).ToLower(), nameof(Version).ToLower()], token: cancellation);
    }
    ...
    public ValueTask<Response<Item>> GetItemAsync(string slug, CancellationToken cancellation = default)
    {
        ...
        return Fusion.GetOrSetAsync(url, async c => await Http.GetFromJsonAsync<Response<Item>>(url, JsonOptions, c), tags: [nameof(Item).ToLower()], token: cancellation)!;
    }
    ...
    public async ValueTask<Response<ItemSet>> GetItemSetAsync(string slug, CancellationToken cancellation = default)
    {
        ...
        var index = await Fusion.GetOrSetAsync(slug, async can =>
        {
            ...
            await Fusion.SetAsync("set:" + set.Data.Id, set, null, [nameof(ItemSet).ToLower(), "set"], token: can);
            foreach (var item in set.Data.Items)
            {
                await Fusion.SetAsync($"https://api.warframe.market/v2/item/{item.Slug}/set", "set:" + set.Data.Id, tags: [nameof(Item).ToLower(), "set", "index"], token: can);
                await Fusion.SetAsync($"https://api.warframe.market/v2/item/{item.Slug}", new Response<Item>(set.ApiVersion, item, null), tags: [nameof(Item).ToLower()], token: can);
            }
            return "set:" + set.Data.Id;
        }, op => { }, [nameof(ItemSet).ToLower(), "set", "index"], cancellation);

        return (await Fusion.GetOrDefaultAsync<Response<ItemSet>>(index, null, token: cancellation))!;
    }
    ...
    {
        return GetSkippingDistributedCacheAsync<Response<Order[]>>("https://api.warframe.market/v2/orders/recent", cancellation);
    }
    ...
    {
        return GetSkippingDistributedCacheAsync<Response<Order[]>>($"https://api.warframe.market/v2/orders/item/{slug}", cancellation);
    }
    ...
    {
        return GetSkippingDistributedCacheAsync<Response<OrderTop>>($"https://api.warframe.market/v2/orders/item/{slug}/top", cancellation);
    }
    /// <summary>
    ...
    public async ValueTask<Response<OrderTop>> GetOrdersItemTopAsync(string slug, OrderTopQueryParameter query, CancellationToken cancellation = default)
    {
        ...
        return await GetSkippingDistributedCacheAsync<Response<OrderTop>>($"https://api.warframe.market/v2/orders/item/{slug}/top?{queryString}", cancellation);
    }
    ...
    {
        return GetSkippingDistributedCacheAsync<Response<Order[]>>($"https://api.warframe.market/v2/orders/user/{slug}", cancellation);
    }
    ...
    {
        return GetSkippingDistributedCacheAsync<Response<User>>($"https://api.warframe.market/v2/user/{slug}", cancellation);
    }
    ...
    public async ValueTask<Response<Statistic>> GetStatisticAsync(string slug, CancellationToken cancellation = default)
    {
        ...
        var softExpiration = TimeSpan.FromHours((utcNow.Hours + 2) / 2 * 2) - utcNow;
        var hardExpiration = TimeSpan.FromDays(2) - utcNow;

        return await Fusion.GetOrSetAsync(
         key: url,
         factory: async (cancellationToken) =>
         {
             ...
             .SetFailSafe(true, hardExpiration),
         token: cancellation
     );
    }
