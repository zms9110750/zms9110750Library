...
public sealed class TreeNode<T>(T value) : IEquatable<T>, IList<TreeNode<T>>
{
	...
	public TreeNode<T>[] AddAt(Index index, params TreeNode<T>[] node)
	{
		if (node == null)
		{
			throw new ArgumentException("node为null");
		}
		foreach (var item in node)
		{
			if (item == null)
			{
				throw new ArgumentException("不能添加空节点");
			}
			if (this <= item)
			{
	...
	public TreeNode<T>? AncestorCommon(TreeNode<T> other)
	{
		if (other == null || Root != other.Root)
		{
			return null;
		}
		Stack<TreeNode<T>> ancestor = Ancestor();
		Stack<TreeNode<T>> otherAncestor = other.Ancestor();
		TreeNode<T> node = null!;
		while (ancestor.TryPop(out var a) && otherAncestor.TryPop(out var b) && a == b)
		{
			node = a;
		}
		return node;
