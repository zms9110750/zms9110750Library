...
public class GraphBuild<TValue, TWeight> where TValue : notnull
{
	private Dictionary<TValue, GraphNodeBuild<TValue, TWeight>> Nodes { get; } = new Dictionary<TValue, GraphNodeBuild<TValue, TWeight>>();

	/// <summary>
	/// 获取所有节点
	/// </summary>
	public IReadOnlyCollection<GraphNodeBuild<TValue, TWeight>> ReadOnlyNodes => Nodes.Values;
	...
	{
		var node = new GraphNodeBuild<TValue, TWeight> { Value = value };
	...
	public void AddDirectedEdge(TValue from, TValue to, TWeight weight)
	{
		...
		fromNode.OutEdges.Add(toNode, weight);
	}
	...
	public GraphNodeBuild<TValue, TWeight>? GetNode(TValue value)
	{
	...
	/// <param name="weight">权重</param> 
	/// <returns></returns>
	public bool ContainsEdge(TValue from, TValue to, [MaybeNullWhen(false)] out TWeight weight)
	{
		weight = default;
		var fromNode = GetNode(from);
		var toNode = GetNode(to);
		return fromNode != null && toNode != null && fromNode.OutEdges.TryGetValue(toNode, out weight);
	}
	...
	/// </summary>
	/// <returns></returns>
	public Graph<TValue, TWeight> Build()
	{
		// 节点映射表：用于快速查找原始节点对应的可变节点
		var nodeMapping = new Dictionary<TValue, GraphNodeMutable<TValue, TWeight>>();

		// 第一遍遍历：创建所有可变节点实例
		// --------------------------------------------
		foreach (var node in Nodes)
		{
			var mutableNode = new GraphNodeMutable<TValue, TWeight>
			{
		...
		foreach (var node in Nodes)
		{
			...
			foreach (var edge in node.Value.OutEdges)
			{
				...
				if (nodeMapping.TryGetValue(edge.Key.Value, out var targetNode))
				{
					// 添加出边（源节点 -> 目标节点）
					sourceNode.OutEdges.Add(targetNode, edge.Value);

					// 同时维护目标节点的入边集合（构建双向关系）
		...
		// 创建可变图并组织节点
		// --------------------------------------------
		var mutableGraph = new GraphMutable<TValue, TWeight>();
		...
		// 计算强连通分量
		// --------------------------------------------
		HashSet<HashSet<GraphNode<TValue, TWeight>>> outReachable = new();
		HashSet<GraphNode<TValue, TWeight>> inReachable = new();

		// 从所有源节点开始DFS遍历（保证覆盖所有连通区域）
		foreach (var source in mutableGraph.SourceNodes.Cast<GraphNodeMutable<TValue, TWeight>>())
		{
			ComponentMutable<TValue, TWeight>? component = null;
		...
		// 处理孤立节点（每个孤立节点自成独立分量）
		foreach (var isolated in mutableGraph.IsolatedNodes.Cast<GraphNodeMutable<TValue, TWeight>>())
		{
			ComponentMutable<TValue, TWeight>? component = null;
	...
	private static void RecalculateOutReachable(
		GraphNodeMutable<TValue, TWeight> node,
		HashSet<HashSet<GraphNode<TValue, TWeight>>> outReachable,
		HashSet<GraphNode<TValue, TWeight>> inReachable,
		GraphMutable<TValue, TWeight> mutableGraph,
		int level,
		ref ComponentMutable<TValue, TWeight>? component)
	{
		...
		if (node.BuildComponent != null)
		{
			...
			// 将当前路径的入边可达性传播到下游
			foreach (var downstreamNode in node.OutReachableNodes.Cast<GraphNodeMutable<TValue, TWeight>>())
			{
		...
		// 递归处理所有邻居节点
		// --------------------------------------------------
		foreach (var neighbor in node.OutEdges.Keys.Cast<GraphNodeMutable<TValue, TWeight>>())
		{
		...
		// 确保组件存在并将当前节点加入组件
		component ??= new ComponentMutable<TValue, TWeight>() { GraphBuild = mutableGraph };
